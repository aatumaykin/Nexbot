...
**Особенности файлового вывода:**

- **Разворачивание `~`**: Путь `~/.nexbot/nexbot.log` автоматически разворачивается в домашнюю директорию пользователя (например, `/Users/username/.nexbot/nexbot.log`)
- **Автоматическое создание директории**: Если директория для лог-файла не существует, она будет создана автоматически с правами `0755`
- **Режим открытия файла**: Лог-файл открывается в режиме добавления (`O_APPEND`), новые записи добавляются в конец файла
...
**Примечания:**
- Файловый вывод полезен для отладки и мониторинга в production
- JSON формат удобен для парсинга логов с помощью инструментов вроде `jq`
- Text формат удобен для чтения человеком в терминале
- При использовании файлового вывода убедитесь, что у пользователя есть права на запись в указанную директорию
