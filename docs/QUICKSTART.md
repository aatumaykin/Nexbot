# Nexbot Quick Start

Быстрое начало работы с Nexbot за 5 минут.

## 🚀 Установка и запуск (5 минут)

### 1. Установка бинарника (2 минуты)

```bash
# Загрузить последний релиз
curl -L https://github.com/aatumaykin/nexbot/releases/latest/download/nexbot-$(uname -s)-$(uname -m) -o nexbot

# Сделать исполняемым
chmod +x nexbot

# Установить в PATH
sudo mv nexbot /usr/local/bin/
```

### 2. Получение API ключей (2 минуты)

**Z.ai API ключ:**
1. Перейти на [Z.ai](https://z.ai)
2. Создать API ключ (начинается с `zai-` или `sk-`)

**Telegram Bot токен:**
1. В Telegram найти [@BotFather](https://t.me/BotFather)
2. Отправить `/newbot`
3. Получить токен (формат: `1234567890:ABCdefGHIjklMNOpqrsTUVwxyz`)

### 3. Настройка конфигурации (30 секунд)

```bash
# Создать директорию
mkdir -p ~/.nexbot

# Создать конфиг
cat > ~/.nexbot/config.toml << 'EOF'
[workspace]
path = "~/.nexbot"

[llm]
provider = "zai"

[llm.zai]
api_key = "${ZAI_API_KEY}"
base_url = "https://api.z.ai/api/coding/paas/v4"
model = "glm-4.7-flash"

[channels.telegram]
enabled = true
token = "${TELEGRAM_BOT_TOKEN}"

[cron]
enabled = true
timezone = "UTC"
jobs_file = "jobs.json"

[subagent]
enabled = true
max_concurrent = 10
timeout_seconds = 300
session_prefix = "subagent-"

[workers]
pool_size = 5
queue_size = 100

[heartbeat]
enabled = true
check_interval_minutes = 30

[logging]
level = "info"
format = "json"

# Вывод логов (опционально)
# - stdout/stderr: вывод в консоль
# - путь к файлу: вывод в файл (например, ~/.nexbot/nexbot.log)
# output = "~/.nexbot/nexbot.log"
EOF

# Установить переменные окружения
export ZAI_API_KEY="ваш-zai-ключ"
export TELEGRAM_BOT_TOKEN="ваш-telegram-токен"
```

### 4. Проверка и запуск (30 секунд)

```bash
# Проверить конфигурацию
nexbot config validate

# Запустить
nexbot serve
```

## 💬 Базовое использование

### Настройка BotFather

1. После создания бота, найдите его в Telegram по username
2. Отправьте тестовое сообщение: `Привет, кто ты?`
3. Бот должен ответить через несколько секунд

### Основные команды

Просто общайтесь с ботом на естественном языке:

```
Привет, кто ты?
Прочитай файл config.toml
Выполни: ls -la
Создай файл test.txt с текстом "Hello World"
```

## ⚙️ Конфигурация v0.2.0

### Новые секции конфигурации

**Cron Scheduler:**
```toml
[cron]
enabled = true              # Включить cron планировщик
timezone = "UTC"           # Часовой пояс для планирования
jobs_file = "jobs.json"    # Файл для хранения задач
```

**Subagent Manager:**
```toml
[subagent]
enabled = true              # Включить subagent spawning
max_concurrent = 10         # Максимальное количество одновременных subagents
timeout_seconds = 300      # Таймаут выполнения задач (по умолчанию 5 минут)
session_prefix = "subagent-"  # Префикс для session ID
```

**Worker Pool:**
```toml
[workers]
pool_size = 5              # Количество воркеров в пуле
queue_size = 100            # Размер буфера очереди задач
```

**HEARTBEAT:**
```toml
[heartbeat]
enabled = true              # Включить HEARTBEAT проверку
check_interval_minutes = 30  # Интервал проверок в минутах
```

```
Привет, кто ты?
Прочитай файл config.toml
Выполни: ls -la
Создай файл test.txt с текстом "Hello World"
```

### Инструменты (Tools)

**Файловые операции:**
```
Прочитай файл ~/.nexbot/config.toml
Создай файл notes.md
Добавь в notes.md строку "Новая заметка"
Покажи список файлов в текущей директории
```

**Shell команды:**
```
Выполни: pwd
Выполни: ls -la
Покажи статус git
```

## ⏰ Cron команды

Автоматическое выполнение задач по расписанию.

### Cron CLI команды

Начиная с v0.2.0, можно управлять cron задачами через CLI:

```bash
# Добавить запланированную задачу
nexbot cron add --schedule "0 9 * * *" --command "Daily health check"

# Список всех запланированных задач
nexbot cron list

# Удалить задачу по ID
nexbot cron remove job_12345
```

### Создание напоминаний через Cron Tool (для агента)

**Обязательный формат для напоминаний:**

```json
{
  "action": "add_oneshot",
  "execute_at": "2026-02-08T01:00:00Z",
  "tool": "send_message",  // REQUIRED: "send_message" или "agent"
  "payload": "{\"message\": \"Reminder text\"}",  // REQUIRED: JSON строка
  "session_id": "telegram:CHAT_ID_HERE"  // REQUIRED для send_message/agent
}
```

**Параметры:**

- `tool` — что выполнять (обязательно): `"send_message"` или `"agent"`
- `payload` — параметры tool (обязательно): `{"message": "текст"}`
- `session_id` — куда отправить (обязательно для send_message/agent): `"telegram:CHAT_ID"`
- `execute_at` — когда выполнять: ISO8601 формат (текущее время + X минут)

**Как работает:**
1. Пользователь просит напоминание → агент создает cron job с tool, payload, session_id
2. Cron job выполняется в назначенное время → вызывает tool с параметрами
3. Сообщение отправляется в правильный Telegram чат

**Устарело:**
- Параметр `command` больше не поддерживается
- Используйте `tool` + `payload` + `session_id` вместо command

**Примеры cron выражений:**

```bash
# Каждый час
nexbot cron add --schedule "0 * * * *" --command "Run hourly status check"

# Каждый день в 9:00
nexbot cron add --schedule "0 9 * * *" --command "Morning health check"

# Каждые 5 минут
nexbot cron add --schedule "*/5 * * * *" --command "Check system status"

# Каждый понедельник в 10:00
nexbot cron add --schedule "0 10 * * 1" --command "Weekly report"

# Каждое первое число месяца в 3:00
nexbot cron add --schedule "0 3 1 * *" --command "Monthly backup"
```

### Создание jobs.json (legacy формат)

Для ручного создания задач можно использовать JSON файл:

```bash
cat > ~/.nexbot/jobs.json << 'EOF'
{
  "jobs": [
    {
      "name": "daily-backup",
      "cron": "0 3 * * *",
      "description": "Ежедневный бэкап в 3:00",
      "enabled": true
    }
  ]
}
EOF
```

### Формат Cron выражений

```
* * * * *
│ │ │ │ │
│ │ │ │ └─── День недели (0-7, 0 и 7 = воскресенье)
│ │ │ └───── Месяц (1-12)
│ │ └─────── День месяца (1-31)
│ └───────── Час (0-23)
└─────────── Минута (0-59)
```

### Частые выражения

| Выражение | Описание |
|-----------|----------|
| `*/5 * * * *` | Каждые 5 минут |
| `0 * * * *` | Каждый час |
| `0 0 * * *` | Ежедневно в полночь |
| `0 3 * * *` | Ежедневно в 3:00 |
| `0 9 * * 1` | Каждый понедельник в 9:00 |
| `30 11 * * 1-5` | В будни в 11:30 |
| `0 0 1 * *` | Первое число каждого месяца |

## 🧬 Spawn Subagent

Создание независимых агентов для параллельного выполнения задач.

### Включение Subagent

Добавьте в `~/.nexbot/config.toml`:

```toml
[subagent]
enabled = true
max_concurrent = 10
timeout_seconds = 300
session_prefix = "subagent-"
```

### Пример диалога с spawn tool

```
Вы: Проанализируй качество кода для репозитория ~/projects/myapp

Bot: Хорошо, я создам subagent для анализа качества кода.

[Bot использует spawn tool]
spawn_task: "Проанализируй качество кода в ~/projects/myapp:
1. Прочитай все .go файлы
2. Проверь соблюдение go best practices
3. Найди потенциальные проблемы
4. Верни отчёт"

[Subagent выполняет задачу параллельно]

Subagent: Анализ кода завершён. Найдено:
- 25 .go файлов
- 3 потенциальные проблемы в обработке ошибок
- 1 проблема с импортами
- Рекомендация: добавить context cancellation для долгих операций

Bot: Анализ качества кода завершён. Subagent нашёл 25 .go файлов и обнаружил 3 проблемы с обработкой ошибок. Основная рекомендация: добавить context cancellation для долгих операций.
```

### Примеры использования

**Пример 1: Анализ репозитория**

```
Используй spawn инструмент для создания subagent:
1. Склонируй репозиторий https://github.com/user/repo.git
2. Найди все .go файлы
3. Проанализируй структуру пакетов
4. Верни отчёт по структуре
```

**Пример 2: Мониторинг логов**

```
Создай subagent для мониторинга:
- Читай ~/.nexbot/nexbot.log
- Ищи ошибки за последний час
- Суммируй найденные ошибки
- Отправь отчёт
```

**Пример 3: Пакетная обработка**

```
Создай subagent для обработки файлов:
- Пройди по всем файлам в ~/Documents
- Прочитай каждый .txt файл
- Извлеки ключевую информацию
- Верни сводку
```

## 💓 HEARTBEAT Задачи

Система health check и proactive задач.

### Создание HEARTBEAT.md

```bash
cat > ~/.nexbot/HEARTBEAT.md << 'EOF'
---
interval: 300
description: Проверка здоровья системы
---

Выполни полную проверку здоровья системы:

1. Проверь статус всех служб
   - Проверь работу SSH
   - Проверь работу HTTP сервера
   - Проверь работу базы данных

2. Проверь дисковое пространство
   - Проверь свободное место на /
   - Проверь свободное место в /home
   - Предупреди, если свободно < 20%

3. Проверь нагрузку системы
   - CPU использование
   - Память использование
   - I/O wait

4. Сгенерируй отчёт
   - Общее состояние (OK/WARNING/CRITICAL)
   - Детали по каждому пункту
   - Рекомендации по устранению проблем
EOF
```

### Формат HEARTBEAT

```yaml
---
interval: 300              # Интервал в секундах (300 = 5 минут)
description: Краткое описание
---

Инструкции для выполнения...
```

### Примеры задач

**Проверка бэкапов:**

```markdown
---
interval: 86400
description: Проверка бэкапов
---

Проверь состояние бэкапов:
1. Проверь существование последних бэкапов
2. Проверь размер бэкапов
3. Проверь дату последнего бэкапа
4. Сообщи, если бэкапы отсутствуют или старые (>7 дней)
```

**Проверка SSL сертификатов:**

```markdown
---
interval: 3600
description: Проверка SSL сертификатов
---

Проверь SSL сертификаты:
1. Проверь срок действия сертификата для example.com
2. Проверь срок действия сертификата для api.example.com
3. Сообщи, если сертификаты истекают в течение 30 дней
```

**Проверка дискового пространства:**

```markdown
---
interval: 600
description: Проверка диска
---

Проверь дисковое пространство:
1. Проверь использование /
2. Проверь использование /home
3. Проверь использование /var
4. Сообщи, если использование > 80%
```

## 🔧 Troubleshooting

### Проблема: "telegram token is required"

**Решение:**
- Замените `ВАШ_TELEGRAM_BOT_TOKEN_ЗДЕСЬ` на реальный токен
- Проверьте, что используете переменную окружения `${TELEGRAM_BOT_TOKEN}`

### Проблема: "Z.ai API key is required"

**Решение:**
- Замените `ВАШ_ZAI_API_КЛЮЧ_ЗДЕСЬ` на реальный API ключ
- Установите переменную окружения: `export ZAI_API_KEY="ваш-ключ"`

### Проблема: "Authentication error"

**Решение:**
- Проверьте, что API ключ правильный и не истёк
- Проверьте, что используете правильный base_url: `https://api.z.ai/api/coding/paas/v4`

### Проблема: Бот не отвечает

**Решение:**
1. Проверьте, что бот запущен:
   ```bash
   ps aux | grep nexbot
   ```

2. Проверьте логи:
   ```bash
   tail -50 ~/.nexbot/nexbot.log
   ```

3. Проверьте конфигурацию:
   ```bash
   nexbot config validate
   ```

4. Проверьте подключение к Telegram:
   ```bash
   curl https://api.telegram.org/bot<ВАШ_ТОКЕН>/getMe
   ```

### Проблема: Cron задачи не выполняются

**Решение:**
1. Проверьте, что cron включён:
   ```toml
   [cron]
   enabled = true
   ```

2. Проверьте файл jobs.json:
   ```bash
   cat ~/.nexbot/jobs.json
   ```

3. Проверьте правильность cron выражений
4. Проверьте логи выполнения:
   ```bash
   grep "cron" ~/.nexbot/nexbot.log
   ```

### Проблема: Subagent не создаётся

**Решение:**
1. Проверьте, что subagent включён:
   ```toml
   [subagent]
   enabled = true
   ```

2. Проверьте лимиты:
   ```toml
   [subagent]
   max_concurrent = 10
   ```

3. Проверьте логи на ошибки создания subagent

### Проблема: HEARTBEAT не выполняется

**Решение:**
1. Убедитесь, что файл HEARTBEAT.md существует в `~/.nexbot/`
2. Проверьте формат frontmatter (должен быть валидный YAML)
3. Проверьте конфигурацию:
   ```toml
   [heartbeat]
   enabled = true
   check_interval_minutes = 10
   ```

4. Проверьте логи:
   ```bash
   grep "HEARTBEAT" ~/.nexbot/nexbot.log
   ```

### Проблема: "Permission denied" при установке

**Решение:**
- Используйте `sudo make install`
- Или установите в домашнюю директорию: `make install-user`

### Проблема: Команды запрещены

**Решение:**
- Добавьте команды в `allowed_commands` в конфиге:
  ```toml
  [tools.shell]
  allowed_commands = ["ls", "cat", "grep", "find", "cd", "pwd", "echo", "date", "git"]
  ```

### Проблема: Доступ к файлам запрещён

**Решение:**
- Добавьте директории в `whitelist_dirs`:
  ```toml
  [tools.file]
  whitelist_dirs = ["~/.nexbot", "~/projects", "~/Documents"]
  ```

## 📚 Дополнительные ресурсы

- **[Полная документация](README.md)** — подробное руководство
- **[Конфигурация](CONFIGURATION.md)** — все настройки
- **[Архитектура](ARCHITECTURE.md)** — устройство системы
- **[Разработка](DEVELOPMENT.md)** — для разработчиков
- **[Примеры](EXAMPLES.md)** — больше примеров использования

## 🆘 Получение помощи

- GitHub Issues: [https://github.com/aatumaykin/nexbot/issues](https://github.com/aatumaykin/nexbot/issues)
- Документация: [https://github.com/aatumaykin/nexbot/tree/main/docs](https://github.com/aatumaykin/nexbot/tree/main/docs)

---

**Happy coding with Nexbot!** 🚀
